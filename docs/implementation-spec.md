...
交付物：达到经用户确认的十类覆盖目标（总计 950+ 类别配额，允许一个单元仅计入一个行为类别）、扩展 Modifier 与 Adapter。
...
- 十类数量是覆盖底线而非质量结论；每条新增单元必须通过语义、上下文、连续性、来源和重复度门禁，禁止用左右镜像、强弱倍率或同义句凑数。
