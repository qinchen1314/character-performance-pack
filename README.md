...
Character Performance Pack 是一个面向**中文小说创作**的本地角色表现引擎，核心目标是让不同角色在相同事件中做出符合身份、彼此可辨且能够长期保持的反应。它综合人物性格、关系、情绪、伤势、持物、场景位置和导演意图生成可解释、可复现的差异化动作；通过角色身份、反应策略和签名行为建立读者能够识别的记忆点与稳定特性；再利用跨场景、章节、卷和全书的行为记忆，减少动作、语义和句法模板重复。

它适合接在写作 Agent、小说工作流或编辑工具之后，专门解决“这个角色此刻会怎样表现，以及为什么只有他会这样表现”的问题。项目不代写剧情和台词；台词由作者提供，工具负责补上符合角色、关系和处境的神态、动作、生理反应与空间行为，并在没有自然动作可用时允许留白，避免为了表现而机械堆叠描写。
