...
当前版本 **0.4.0：高覆盖原创表现目录**。已有 66 种情绪、1024 个表现单元、56 条可配置修正器、SQLite 连续性与中文渲染。核心无需联网或模型 API Key。
...
- 目录已达到：103 微表情、161 面部/视线、315 身体动作、115 空间行为、105 生理反应、111 言语模式、33 人格修正器、19 关系修正器、110 修仙表现。4 个显式导航控制单元不计入空间目标。构建门禁检查数量、完全重复、近重复和每种情绪的候选/通道覆盖；现有第三方来源策略继续限制商业和再分发构建，扩展条目及中文语法为项目原创。
- 物品、前态、环境和话轮信息通过 `context.facts` 显式授权；`held.*` 事实还必须与 `scene_state.held_objects/object_tags` 一致。事实缺失时条目不会进入候选，防止凭空出现杯子、泪水、引语或法器。
...
评估生成 [指标与20回合样本](docs/evaluation/latest.md)、[目录质量报告](docs/evaluation/catalog-quality.json)及机器可读 JSON。本轮扩展见 [0.4 阶段报告](docs/phase-reports/catalog-0.4.md)和[研究记录](docs/research/catalog-sources.md)；连续动作设计见 [0.3 阶段报告](docs/phase-reports/continuity-0.3.md)。
